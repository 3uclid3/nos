...
void pic::init()
{
    ...
    const u8_t md = _master.read(data_offset);
    const u8_t sd = _slave.read(data_offset);
...
void pic::mask_irq(u8_t irq)
{
    ...
    const u8_t value = port.read(data_offset) | static_cast<u8_t>(1 << irq);
    port.write(value, data_offset);
}

void pic::unmask_irq(u8_t irq)
{
    auto& port = irq < 8 ? _master : _slave;
