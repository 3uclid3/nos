...
extern "C" void nos_interrupt_handler(nos::x86_64::cpu_registers* registers)
{
...
{
    const auto exception_count = to_underlying(details::exception::count);
...
{
    log::error("isr: exception interrupt {:X} on CPU {}", registers.interrupt, 0);
